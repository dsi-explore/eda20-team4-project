...
scaled_salary_data_jc %>% ggplot(aes(y = salary, x = job_category, fill = type)) +
  ...
  theme_classic() +
   coord_flip()
```
It does not make sense to compare the salary ranges of certain job types such as full time versus part time so we will look at only jobs that are typically a full 40 hours a week.

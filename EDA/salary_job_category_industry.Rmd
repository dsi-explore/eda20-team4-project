...
We want to filter the data set for only full time data science related jobs in New York City and look at the distribution of minimum and maximum salary as well as average salaries across different data science related roles. For this analysis we are using the salary, not the scaled salary, as we are only look at each metro location separately and do not need to account for cost of living in the comparison.

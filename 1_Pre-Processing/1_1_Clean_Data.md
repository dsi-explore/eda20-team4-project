...
The unit of analysis in the `ds_jobs` data set is a specific job opening
within the field of data science at a company listed on glassdoor.
...
`TX` and we need to be consistent with abbreviated states throughout the
data set.
...
We want to create a `job_category` variable that will group different
roles within data science. Currently there are only `job_title` and
`job_desc` variables that are text. To identify patterns of data science
roles and group similar jobs together, we performed text analysis on
these two variables.

Here we are finding common data science words or roles within the
`job_title` variable.
...
Now we are creating a new variable `job_category` that groups job
openings into similar roles by the above text analysis.
